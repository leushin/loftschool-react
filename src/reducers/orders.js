import { CREATE_NEW_ORDER, createNewOrder } from '../modules/clients';
...
export default (state = [], action) => {
    switch (action.type) {
            ...
            return state.map(order => {
                if (order.id === action.payload) {
                    switch (order.position) {
                        ...
                        case 'conveyor_4':
                            const isComplete = order.recipe.every(i => order.ingredients.includes(i));
                            if (isComplete) {
        ...
        case ADD_INGREDIENT: {
            const firstOrder = state.find(order =>order.position === action.payload.from);
            if (!firstOrder) {
                return state;
            }
            return state.map(order => {
